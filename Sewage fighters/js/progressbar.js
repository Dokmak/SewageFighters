...
var presentsDone = false ; 

window.onload = function() {
	var elem = document.getElementById('canvas_progressBar');
...
function progressText(ctx, x, y, width, height, radius, max) {
	ctx.save();
	ctx.fillStyle = 'white';
	var progNumber = Math.floor(width/max*100);
	var text = progNumber+"%";
	...
	if(progNumber == 100)
	{
		$(".canvasWrap,.imageBack1").fadeOut(1500,function(){
			 $(this).detach();
		});	
		// Start the game 
		progressDone = true;
	...
	{
		$('#wrapper').prepend("<div class='canvasWrap' id='texts' style='margin-top:180px; margin-left:145px;'><h3>Presents</h3></div>");
		$('.canvasWrap').fadeIn(4000);
		presentsDone = true;	
	}
	
	if(presentsDone == true)
	{
		$(".canvasWrap").fadeOut(5000,function(){
			$("#wrapper").append("<div id='temp' title='true'> </div>");
			$(this).detach();
		});
	}
